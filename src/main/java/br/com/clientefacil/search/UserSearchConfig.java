...
public final class UserSearchConfig {

    private UserSearchConfig() {
    }
    ...
    public static final Map<String, SearchField> FILTER_FIELDS = Map.of(
            ...
            "email", new SearchField("email", SearchFieldType.STRING),
            "personId", new SearchField("person.id", SearchFieldType.LONG),
            "profileId", new SearchField("profile.id", SearchFieldType.LONG),
            "companyId", new SearchField("company.id", SearchFieldType.LONG)
    );
    ...
    public static Specification<User> byFilters(List<FilterRequest> filters) {
